...
**Theorem**: Let `D` be a deposit operation and `W` be a withdrawal operation in the DarkSight protocol. With anonymity set size `k ≥ 2`, synthetic activity ratio `ρ ≥ 1.0`, temporal mixing window `Δt ∈ [1h, 24h]`, and denomination granularity `δ = 0.1 SOL`, the probability of an adversary correctly linking `D` to `W` is bounded by:
