/**
 * DarkSight SDK Main Class
 * 
 * Status: Alpha - Logic Implementation
 */

import { DarkSightSDKOptions, CreateMarketOptions, Market, CreatePositionOptions, Position, DepositResult, WithdrawOptions, WithdrawResult, GetMarketsOptions, GetPositionsOptions } from './types';
import { CryptoUtils } from './crypto';
import { MerkleTree } from './merkle';
import { PublicKey, Transaction } from '@solana/web3.js';

export class DarkSightSDK {
  ...
  async initialize(): Promise<void> {
    ...
    console.log('Initializing DarkSight SDK...');
  ...
  private checkInit() {
    if (!this.initialized) throw new Error('DarkSight SDK not initialized');
  }
}
